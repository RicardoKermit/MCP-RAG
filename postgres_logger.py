...
class PostgresLogger:
    ...
    def _setup_file_logger(self) -> logging.Logger:
        ...
        file_handler = logging.FileHandler(
            f"logs/postgres_logger_{datetime.now().strftime('%Y%m%d')}.log"
        )
        ...
        try:
            with self.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    """, (operation_type, datetime.now().date()))
                    ...
                    if existing:
                        ...
                        cursor.execute("""
                            UPDATE operation_stats SET
                                ...
                                error_count = %s,
                                avg_duration_ms = %s,
                                min_duration_ms = LEAST(min_duration_ms, %s),
                                max_duration_ms = GREATEST(max_duration_ms, %s),
                                updated_at = %s
                            WHERE id = %s
                        """, (
                            new_total, new_success, new_error, new_avg,
                            duration_ms, duration_ms, datetime.now(), stats_id
                        ))
                    else:
    ...
    def _log_to_file(self, log_type: str, data: Dict[str, Any]):
        ...
        log_file = self.logs_dir / f"fallback_{log_type}_{datetime.now().strftime('%Y%m%d')}.log"
        try:
            with open(log_file, 'a', encoding='utf-8') as f:
                f.write(f"{datetime.now().isoformat()} - {log_type.upper()} - {json.dumps(data)}\n")
        except Exception as e:
